...
{
    ui->setupUi(this);
    this->setWindowFlags(this->windowFlags()&~Qt::WindowMaximizeButtonHint);//禁止窗口放大
    connect(ui->horizontalSlider,&QSlider::valueChanged,this,&selectkind::slot_valueChanged);
    connect(this,&selectkind::sig_valueChanged,m_player,&player::slot_value);//滑动控制音量
    initsys();
}
